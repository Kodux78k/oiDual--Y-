...
function saveConversationNow(){
  try{
    const snap = {
      ...
      conversation: conversation.slice(0,100) // limite razoável
    };
    const key = 'KDX_CONV_' + snap.ts;
    localStorage.setItem(key, JSON.stringify(snap));
    // também manter um index
    ...
    footerHint.textContent = 'Conversa salva.';
    renderHistoryItems();
    return true;
  }catch(e){
    console.error(e);
    footerHint.textContent = 'Erro ao salvar conversa.';
    return false;
  }
}
  ...
  container.querySelectorAll('[data-load]').forEach(btn=>{
    btn.addEventListener('click', (ev)=>{
      ...
      if (snap){
        ...
        // re-render UI básico: limpa e injeta blocos simples (adapte se tiver função render própria)
        responseList.innerHTML = '';
        conversation.slice().reverse().forEach(item=>{
          const d = document.createElement('div');
          d.className = 'response-block middle';
          d.innerHTML = `<div><strong>${item.role}</strong><div style="opacity:.85;margin-top:6px">${escapeHtml(item.content || item.content)}</div></div>`;
          responseList.appendChild(d);
        });
        footerHint.textContent = 'Conversa carregada do histórico.';
      }
    });
  });
...
// auto-save hook: salva quando conversa muda
...
// chamar maybeAutoSave sempre que push em conversation
// recomenda inserir onde sua app empurra mensagens:
// exemplo (no seu fluxo onde append na conversation, logo após push:)
function pushConversation(item){
  conversation.unshift(item);
  // se tiver uma função de render, chamá-la; senão jogamos no responseList
  maybeAutoSave();
}
    ...
    // ---------- substituir handler do themeToggle por abrir o painel de Config ----------
...
function ensureSettingsPanel(){
  ...
  document.getElementById('kdxVoiceApply').addEventListener('click', ()=>{
    ...
    if (v){
      // set manual override into voiceConfig.default
      voiceConfig = voiceConfig || {};
      voiceConfig.default = voiceConfig.default || {};
      // store a voiceHint so speakBlock will pick it
      ...
      footerHint.textContent = `Voz sobrescrita: ${v}`;
    } else {
      footerHint.textContent = 'Voz automática por arquétipo.';
      // remove manual default
      if (voiceConfig && voiceConfig.default) { delete voiceConfig.default; localStorage.setItem(STORAGE.VOICE_CONFIG, JSON.stringify(voiceConfig)); }
    }
  });
...
// botão #themeToggle agora abre painel de configs
...
// Config (ajuste aqui)
// -------------------
const SPEAK_COUNT = 9; // padrão: últimos 9 blocos (use 1, 9, etc)
const PAUSE_BETWEEN_BLOCKS_MS = 120; // pausa entre blocos (visível)
...
// -------------------
// TTS original (mantido sem alteração)
// -------------------
...
// - não altera speakBlock, apenas observa currentUtterance.onend
// - funciona também quando SpeechSynthesis não existe (heurística timed fallback)
// -------------------
function speakBlockPromise(block){
  return new Promise((resolve) => {
    ...
    // se a API existe e a função speakBlock irá criar currentUtterance:
    if (typeof synth !== 'undefined' && synth && 'speak' in synth && typeof speakBlock === 'function'){
      // chama a tua função (ela define currentUtterance internamente)
      try {
        // Guardamos referência para o utterance que será criada
        // Depois do speakBlock ser chamado, currentUtterance deverá apontar para o novo utter.
        speakBlock(block);
      } catch (e){
      ...
      if (utter){
        ...
        // currentUtterance não foi definido (improvável) -> fallback timed
    ...
    // finalização comum: remove destaque e resolve
    function finish(){
...
// -------------------
// Sequência: fala n blocos em ordem (usa speakBlockPromise)
// - count: número de blocos (Infinity = todos)
...
// -------------------
// Compatibilidade com função antiga speakLastBlock (agora chama speakRecentBlocks(1))
// -------------------
function speakLastBlock(){
  // comportamento legado: fala apenas o último bloco
  speakRecentBlocks(1);
}

// -------------------
// Botão de voz: clique padrão fala últimos SPEAK_COUNT blocos
...
// -------------------
// onBlockClick: mantém tua lógica, mas usa speakBlock (comportamento existente)
// -------------------
function onBlockClick(ev){
  ...
  if (state === 'idle' || state === 'sent'){
    block.dataset.state = 'spoken';
    // mantém chamada direta ao speakBlock (como já faz)
    speakBlock(block);
...
// -------------------
// addTtsButtonToBlock: mantém comportamento (usa speakBlock)
// -------------------
function addTtsButtonToBlock(div){
  ...
  btn.addEventListener('click',(ev)=>{
    ev.stopPropagation();
    // mantemos a chamada direta à tua função original
    speakBlock(div);
  });
  div.appendChild(btn);
}
...
// -------------------
// enhanceBlock: igual (só garante push no array)
// -------------------
...
// -------------------
// appendBlocksFromData: ao anexar blocos, fala os últimos SPEAK_COUNT por padrão
// -------------------
function appendBlocksFromData(blocks){
  ...
  // scrollToBottom();
  // por padrão fala os últimos SPEAK_COUNT blocos
  speakRecentBlocks(SPEAK_COUNT);
}

// -------------------
// appendUserPulseBlock: mantém adição do bloco do usuário (não fala automaticamente)
// -------------------
function appendUserPulseBlock(text){
  ...
  enhanceBlock(div);
  responseList.appendChild(div);
  // por padrão não fala automaticamente o bloco do usuário
}
