 ...
 *
 * Separação:
 *  • createSessionFrame()  -> navegação iframe com double-click
 *  • koblluxSync()         -> sincronização com remote JSON
 *  • Namespace unificado   -> KOBLLUX_BRIDGE + KOBLLUX_DOCK
 */
'use strict';
...
function ensureUnifiedNamespace() {
  ...
  root.version = root.version || '∆7';
  root.law = root.law || 'VERDADE × INTEGRAR ÷ Δ = ∞';
...
/* -------------------------------------------------------
   1) koblluxSync() — sincronização com remote JSON
------------------------------------------------------- */
async function koblluxSync(
  url = 'https://truetruextrue.github.io/JESUS_VERB-/output/KOBLLUX_N8N_BRIDGE.json'
) {
  try {
    ...
    if (window.ARCHETYPE_KEYWORDS) {
      ...
      console.log(
        `[∆7] ARCHETYPE_KEYWORDS enriquecido · ${Object.keys(v).length} arquétipos`
      );
    }
    ...
    console.warn('[KOBLLUX] Sync offline:', e.message);
    return null;
  }
}

/* -------------------------------------------------------
   2) createSessionFrame() — iframe com double-click
------------------------------------------------------- */
...
@keyframes kblxPop{
  ...
  document.addEventListener('click', e => {
    if (!p.contains(e.target) && !e.target.closest('.symbol-button')) {
    ...
    ({
      symbol: () => {
        ...
        window.dispatchEvent(
          new CustomEvent('nagatanazare:orb-inject', {
...
(function init() {
  ...
  window.KOBLLUX = { ...window.KOBLLUX, ...api };
  window.KOBLLUX_BRIDGE = window.KOBLLUX;
  window.KOBLLUX_DOCK = window.KOBLLUX;

  console.log('[∆7] namespace unificado disponível: window.KOBLLUX');
})();
