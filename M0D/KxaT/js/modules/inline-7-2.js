...
*/
  ...
  function speakBlockPromise(block){
    return new Promise((resolve) => {
      ...
      try { scrollBlockIntoView(block); } catch(e){}
  ...
  if (typeof bootBlock !== 'undefined' && bootBlock) {
    bootBlock.remove();
    bootBlock = null;
  }
  ...
  blocks.forEach(page => {
    ...
    if (typeof enhanceBlock === 'function') enhanceBlock(div);

    responseList.appendChild(div);
    
    // Gerenciamento de Memória (Evita array infinito e degradação de performance)
    if (typeof responseBlocks !== 'undefined' && !responseBlocks.includes(div)) {
      responseBlocks.push(div);
      if (responseBlocks.length > 369) {
        responseBlocks.splice(0, responseBlocks.length - 369);
      }
    }
  });

  // Rola UMA VEZ após a inserção em lote
  scrollResponseToBottom(true);

  // Inicia o TTS (agora sem competir com o scroll)
  if (typeof speakRecentBlocks === 'function') {
    speakRecentBlocks(SPEAK_COUNT);
  }
}
