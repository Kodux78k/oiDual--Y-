(function(){
  ...
  const themeToggleBtn = $('#kdxThemeToggle');
  ...
  function scrollBlockIntoView(block){
    if (!block) return;
    if (!__kdxAutoScrollEnabled) return;
    try{
  ...
  function maybeAutoSave(){
    if (localStorage.getItem('KDX_AUTOSAVE') === '1'){
      ...
      window.__kdx_auto_save_t_ = setTimeout(saveConversationNow, 1200);
    }
  }
  ...
  function ensureSettingsPanel(){
    ...
    voiceSel.innerHTML = '<option value="">Automático (por arquétipo)</option>' +
      archKeys.map(k=>`<option value="${k}">${k}</option>`).join('');
  ...
  function openSettingsPanel(){
    ...
    document.body.dataset.pageStyle = localStorage.getItem('PAGE_STYLE') || 'default';
  }
  ...
  function parseMarkdownBasic(rawText){
    ...
    text = text.replace(/```([\s\S]*?)```/g, (_m, code) => '<pre><code>' + code.replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</code></pre>');
  ...
  function speakBlock(block){
    ...
    utter.lang = profile?.lang || 'pt-BR';
    utter.rate = typeof profile?.rate === 'number' ? profile.rate : 1;
    utter.pitch = typeof profile?.pitch === 'number' ? profile.pitch : 1;
    utter.volume = typeof profile?.volume === 'number' ? profile.volume : 1;
    ...
    if (profile && profile.voiceHint && availableVoices && availableVoices.length){
      ...
      if (!chosen) chosen = availableVoices.find(v => v.lang === utter.lang) || null;
  ...
  function speakBlockPromise(block){
    return new Promise((resolve) => {
      ...
      const text = (typeof getBlockText === 'function')
        ? getBlockText(block)
  ...
  async function speakRecentBlocks(count = SPEAK_COUNT, opts = {}){
    ...
    for (let i = 0; i < blocks.length; i++){
      ...
      await new Promise(r => setTimeout(r, opts.pauseBetween || PAUSE_BETWEEN_BLOCKS_MS));
    }
  }
  ...
  async function sendPrompt(promptText, options){
    ...
    footerHint.textContent = fromBlock ? 'Pulso em expansão a partir do bloco.' : 'Processando pulso.';
  ...
  copyBtn.addEventListener('click', async ()=>{
    ...
    try{
      ...
      footerHint.textContent = 'Bloco copiado inteiro.';
    }catch(e){
      try{
        await navigator.clipboard.writeText(target.innerText);
        footerHint.textContent = 'Copiei só o texto do bloco.';
      }catch{
  ...
  if (toggleLoginBtn){
    toggleLoginBtn.addEventListener('click', ()=> loginBox.classList.toggle('active'));
  }
  ...
  if (settingsBtn && iaConfigPanel){
    settingsBtn.addEventListener('click', ()=> iaConfigPanel.classList.toggle('active'));
  }
      ...
      reader.onload = ()=>{
        ...
        if (name.endsWith('.css')){
          ...
          try{
            ...
            fn(window, document);
            footerHint.textContent = 'Parser JS carregado.';
          }catch(e){
  ...
  if (copyBtn){
    copyBtn.addEventListener('click', async ()=>{
      ...
      try{
        await navigator.clipboard.writeText(target.innerText);
        footerHint.textContent = 'Bloco copiado.';
      }catch{
        footerHint.textContent = 'Não consegui copiar automaticamente.';
      }
    });
