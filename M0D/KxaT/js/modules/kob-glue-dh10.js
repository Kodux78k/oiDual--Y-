// kob-glue-dh10.js — documentElement arch root carrier
...
(function(){
  ...
  /* ─────────────────────────────────────────────
     ROOT LAYER — three levels, all kept in sync
     ┌─ document.documentElement  ← data-arch (CSS :root selector source-of-truth)
     │                              data-voice-arch (compat)
     ├─ document.body             ← data-voice-arch  (legacy compat)
     └─ meta[name="kob-arch"]     ← content attr     (meta-layer readable)
  ───────────────────────────────────────────── */
  const HTML  = document.documentElement; // <html> — ROOT ABSOLUTE
  const BODY  = document.body;
  const META_ARCH = document.querySelector('meta[name="kob-arch"]');

  /* ─────────────────────────────────────────────
     UI ELEMENTS — tolerant selectors
     frame: #content-frame → #frame → #navFrame → any <iframe>
     root:  #root → <body>   (scan root, not arch root)
  ───────────────────────────────────────────── */
  ...
  // root = content scan root (NOT the arch root — see HTML above)
  ...
  ───────────────────────────────────────────── */
  const ARCHETYPES = [
    { id:'kobllux',  name:'KOBLLUX',  voice:'Luciana', lang:'pt-BR', rate:0.98, pitch:0.39, color:'#22D3EE',
    ...
    { id:'kodux',    name:'KODUX',    voice:'Rocko',   lang:'pt-BR', rate:0.86, pitch:0.18, color:'#F97316',
    ...
    { id:'atlas',    name:'ATLAS',    voice:'Reed',    lang:'en-US', rate:1.00, pitch:0.78, color:'#78e3ff',
    ...
    { id:'nova',     name:'NOVA',     voice:'Luciana', lang:'pt-BR', rate:1.06, pitch:1.39, color:'#ff6b6b',
    ...
    { id:'vitalis',  name:'VITALIS',  voice:'Rocko',   lang:'pt-BR', rate:0.96, pitch:1.39, color:'#4ecdc4',
    ...
    { id:'pulse',    name:'PULSE',    voice:'Reed',    lang:'pt-BR', rate:1.00, pitch:1.48, color:'#a8e6cf',
    ...
    { id:'artemis',  name:'ARTEMIS',  voice:'Paulina', lang:'es-MX', rate:1.00, pitch:1.23, color:'#ffd93d',
    ...
    { id:'serena',   name:'SERENA',   voice:'Joana',   lang:'pt-BR', rate:0.92, pitch:0.90, color:'#b8e1ff',
    ...
    { id:'kaos',     name:'KAOS',     voice:'Rocko',   lang:'pt-BR', rate:1.09, pitch:1.37, color:'#ff8066',
    ...
    { id:'genus',    name:'GENUS',    voice:'Reed',    lang:'pt-BR', rate:0.98, pitch:1.23, color:'#95e1d3',
    ...
    { id:'lumine',   name:'LUMINE',   voice:'Flo',     lang:'fr-FR', rate:1.03, pitch:1.78, color:'#f9f3b2',
    ...
    { id:'solus',    name:'SOLUS',    voice:'Satu',    lang:'fi-FI', rate:0.99, pitch:0.78, color:'#ffb347',
    ...
    { id:'rhea',     name:'RHEA',     voice:'Alice',   lang:'it-IT', rate:1.02, pitch:0.45, color:'#b5eaea',
    ...
    { id:'aion',     name:'AION',     voice:'Milena',  lang:'ru-RU', rate:0.88, pitch:0.30, color:'#c79aff',
    ...
    { id:'uno',      name:'UNO',      voice:'Grandma', lang:'en-US', rate:0.90, pitch:0.13, color:'#f97316',
    ...
    { id:'dual',     name:'DUAL',     voice:'Luciana', lang:'pt-BR', rate:1.02, pitch:1.02, color:'#06b6d4',
    ...
    { id:'trinity',  name:'TRINITY',  voice:'Sandy',   lang:'en-US', rate:1.04, pitch:0.33, color:'#ec4899',
    ...
    { id:'horus',    name:'HORUS',    voice:'Flo',     lang:'it-IT', rate:1.24, pitch:0.14, color:'#f59e0b',
    ...
    { id:'bllue',    name:'BLLUE',    voice:'Flo',     lang:'fr-FR', rate:1.01, pitch:1.10, color:'#60a5fa',
      theme:{ primary:'#60a5fa', secondary:'#93c5fd', bgSoft:'radial-gradient(circle at 45% 25%,rgba(96,165,250,.08),transparent)', glow:'0 0 18px rgba(96,165,250,.55)' } }
  ];
  ...
  (function attachSymbolBarHandler(){
    ...
    bar.addEventListener('click', ev=>{
      ...
      const callTTS = fnName => {
        ...
        }catch(err){ console.warn('callTTS error', err); }
        return false;
      };
  ...
  /* ─────────────────────────────────────────────
     applyVoiceTheme
     Writes to THREE layers:
       1. document.documentElement  ← data-arch  (CSS :root selector truth)
       2. document.body             ← data-voice-arch  (legacy compat)
       3. meta[name="kob-arch"]     ← content (meta-layer)
       4. CSS custom properties     ← :root vars (JS-driven, complement CSS rules)
  ───────────────────────────────────────────── */
  function applyVoiceTheme(arch){
    ...
    // ── LAYER 1: html[data-arch] — the CSS selector root ──
    HTML.setAttribute('data-arch',       arch.id);
    HTML.setAttribute('data-voice-arch', arch.id);  // compat alias on html

    // ── LAYER 2: body[data-voice-arch] — legacy compat ──
    BODY.setAttribute('data-voice-arch', arch.id);

    // ── LAYER 3: meta kob-arch ──
    if(META_ARCH) META_ARCH.setAttribute('content', arch.id);

    // ── LAYER 4: CSS custom properties (supplement, not replace CSS rules) ──
    const root = document.documentElement; // same as HTML — explicit for clarity
    ...
    // TTS system vars (used by outline, dock)
  ...
  function updateArchetype(idx){
    state.archIdx = (typeof idx==='number') ? (((idx % ARCHETYPES.length) + ARCHETYPES.length) % ARCHETYPES.length) : 0;
    ...
    }catch(e){ console.warn('updateArchetype fail', e); }
  ...
  function scanBlocks(){
    ...
    const localNodes = [
      ...(root.querySelectorAll ? root.querySelectorAll('h1,h2,h3,p,li,blockquote,pre,td,th') : [])
  ...
  function setStatus(){
    const el = $('#tts-status');
    if(!el) return;
    el.textContent = !state.blocks.length ? '0/0' : `${Math.min(state.currentBlockIdx+1,state.blocks.length)}/${state.blocks.length}`;
  }

  function showOutlineFor(node){
    if(!outline || !node){ if(outline) outline.style.display='none'; return; }
    try{
  ...
  ───────────────────────────────────────────── */
  $('#tts-on')    && $('#tts-on').addEventListener('click',    ()=> state.isSpeaking ? stopSpeech() : startSpeech());
  ...
  $('#tts-sel') && $('#tts-sel').addEventListener('click', ()=>{
    ...
    if(voice) u.voice=voice;
    if(arch.lang) u.lang=arch.lang;
    u.rate=arch.rate??1; u.pitch=arch.pitch??1;
    synth.speak(u);
  });
  ...
  Object.assign(window.KOBLLUX, { startSpeech, stopSpeech, rebuildBlocks, updateArchetype, applyVoiceTheme, state });
  ...
  window.KOBLLUX.speakText = window.KOBLLUX.speakText || function(txt, opts){
    try{
      ...
      const pickVoice=()=>{ try{ const voices=synthLocal.getVoices()||[]; if(voiceName){ const f=voices.find(v=>v.name&&v.name.toLowerCase().includes(String(voiceName).toLowerCase())); if(f) return f; } return voices.find(x=>/pt/i.test(x.lang))||voices[0]; }catch{return null;} };
  ...
  /* ─────────────────────────────────────────────
     injectVoiceThemeCSS
     Injects baseline :root vars + transition CSS.
     The html[data-arch] CSS rules in the HTML <style>
     handle the actual theme switching via selectors —
     this just ensures the vars exist as fallback.
  ───────────────────────────────────────────── */
...
`;
    ...
    // baseline vars (used when CSS selector rules haven't fired yet)
...
`;
  ...
  // Expose root references for external scripts
  window.KOBLLUX.htmlRoot       = HTML;   // document.documentElement
  window.KOBLLUX.getArchRoot    = ()=> HTML.getAttribute('data-arch');

  /* ─────────────────────────────────────────────
     INIT — set arch from localStorage or default
  ───────────────────────────────────────────── */
  try{
    // restore last arch from storage
    const savedArch = localStorage.getItem('kob_arch_id');
    const savedIdx  = savedArch ? ARCHETYPES.findIndex(a=> a.id===savedArch) : -1;
    updateArchetype(savedIdx >= 0 ? savedIdx : state.archIdx);
  }catch(e){ console.warn('initial updateArchetype failed', e); }

  console.log('KOBLLUX glue init ✓ | html[data-arch] root active');
...
(() => {
  ...
  window.addEventListener('KOBLLUX_VOICES_READY', ()=>{
    ...
    window.KOBLLUX.send('VOICES_READY', { total: Object.keys(window.KOBLLUX_VOICES||{}).length });
  });

  /* hook updateArchetype — propaga change via postMessage */
  if(window.KOBLLUX.updateArchetype){
    const _old = window.KOBLLUX.updateArchetype;
    window.KOBLLUX.updateArchetype = function(idx){
      _old.call(this, idx);
      // persist arch id
      const arch = window.KOBLLUX.getArchetypes && window.KOBLLUX.getArchetypes()[idx];
      if(arch) try{ localStorage.setItem('kob_arch_id', arch.id); }catch{}
  ...
  if(window.KOBLLUX.speakText){
    const _oldSpeak = window.KOBLLUX.speakText;
    window.KOBLLUX.speakText = function(){
      window.KOBLLUX.send('SPEAK', { text: arguments[0] });
      return _oldSpeak.apply(this, arguments);
    };
  }

  /* ping vivo */
  setInterval(()=> window.KOBLLUX.send('PING', { time: Date.now() }), 5000);

  /* boot */
  window.addEventListener('DOMContentLoaded', ()=>{
    window.KOBLLUX.send('READY', {
      title: document.title,
      archs: window.KOBLLUX.getArchetypes ? window.KOBLLUX.getArchetypes().length : 0
    });
  });
})();
