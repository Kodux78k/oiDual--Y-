    ...
    return new Promise((resolve) => {
      ...
      if (typeof synth !== 'undefined' && synth && 'speak' in synth && typeof speakBlock === 'function'){
        try {
          ...
          const estMs = Math.min(Math.max(text.length * 35, 3500), 312000);
        ...
        if (utter){
          ...
          const safetyTimeout = Math.min(Math.max(text.length * 960, 42000), 120000);
          setTimeout(()=>{ if (!called){ called = true; finish(); } }, safetyTimeout);
          return;
        } else {
          const estMs = Math.min(Math.max(text.length * 40, 3500), 12000);
          setTimeout(finish, estMs);
          return;
        }
      } else {
